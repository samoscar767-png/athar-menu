(function () {
  ...
  // Only allow http(s), data:image, and same-origin/relative URLs through.
  // Anything else (javascript:, unexpected schemes) is dropped, since this
  // data comes from the admin API and ends up in every visitor's page.
  // Returns the raw (unescaped) URL — callers decide how to use it:
  // escapeHtml() it themselves when concatenating into an HTML string, or
  // use it as-is for direct DOM property assignment (e.g. img.src = …),
  // where HTML-entity-escaping would corrupt URLs containing "&".
  ...
  function cardHtml(item) {
    ...
    var defaultImage = escapeHtml(safeImage(hasGallery ? item.variants[0].image : item.image));
    ...
    if (item.variants.length === 1) {
      ...
      priceHtml =
        ...
        item.variants
          .map(function (v, i) {
            var safeVariantImage = escapeHtml(safeImage(v.image));
            var imgAttr = hasGallery && safeVariantImage ? ' data-image="' + safeVariantImage + '"' : "";
  ...
  function renderPromo() {
    ...
    var safePromoImage = safeImage(promo.image);
    if (safePromoImage) {
      image.src = safePromoImage;
      image.alt = currentLang === "ar" ? (promo.nameAr || promo.name) : promo.name;
    }
  }

  // Skip the autoplaying hero video for people on slow/metered connections
  // or who've asked the OS to reduce motion — the poster frame (already
  // loaded via preload="metadata") stands in for it instead.
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    initNewsletter();
    initYear();
    if (window.ZadData && window.ZadData.syncNow) window.ZadData.syncNow();
  });
