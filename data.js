(function () {
  ...
  // Auth is now a short-lived httpOnly session cookie issued by /api/login
  // (see lib/auth.js) — there is no client-visible token anymore. Every
  // fetch that needs the cookie sent must use credentials: "include".
  ...
  const demo = {
    sections: [
      ...
      { id: 2, name: "Flavored Coffee", nameAr: "قهوة منكهة", description: "hot, iced, blended or canned", descriptionAr: "ساخن، بارد، مخلوط أو معلب", active: true, order: 1 },
      { id: 3, name: "Non Coffee", nameAr: "بدون قهوة", description: "no coffee, all flavor", descriptionAr: "بدون قهوة وطعم رائع", active: true, order: 2 },
      { id: 4, name: "Matcha", nameAr: "ماتشا", description: "vibrant green tea", descriptionAr: "شاي أخضر منعش", active: true, order: 3 },
      { id: 5, name: "Smoothies & Fresh Juice", nameAr: "سموذي وعصائر طازجة", description: "fresh fruit blends", descriptionAr: "مزيج فواكه طازجة", active: true, order: 4 },
      { id: 6, name: "Mojito", nameAr: "موهيتو", description: "mint & refreshing", descriptionAr: "نعناع ومنعش", active: true, order: 5 },
      { id: 7, name: "Soft Drinks", nameAr: "مشروبات غازية", description: "cold & fizzy", descriptionAr: "باردة ومنعشة", active: true, order: 6 },
      { id: 8, name: "Bakery and Dessert", nameAr: "مخبوزات وحلويات", description: "fresh baked & sweet treats", descriptionAr: "مخبوزات طازجة وحلويات لذيذة", active: true, order: 7 }
    ],
    items: [
      ...
      { id: 16, sectionId: 2, name: "Caramel Coffee Frappe", nameAr: "فرابيه قهوة بالكراميل", description: "blended coffee + caramel", descriptionAr: "قهوة مخلوطة + كراميل", image: IMG.frappe, available: true, featured: false, variants: [v("Frappe", 125, null, "فرابيه")] },
      { id: 17, sectionId: 2, name: "Chocolate Coffee Frappe", nameAr: "فرابيه قهوة بالشوكولاتة", description: "blended coffee + chocolate", descriptionAr: "قهوة مخلوطة + شوكولاتة", image: IMG.frappe, available: true, featured: false, variants: [v("Frappe", 125, null, "فرابيه")] },
      { id: 18, sectionId: 2, name: "Lotus Coffee Frappe", nameAr: "فرابيه قهوة لوتس", description: "blended coffee + lotus biscoff", descriptionAr: "قهوة مخلوطة + لوتس بيسكوف", image: IMG.frappe, available: true, featured: false, variants: [v("Frappe", 125, null, "فرابيه")] },
      ...
      { id: 33, sectionId: 8, name: "Plain Croissant", nameAr: "كرواسون عادي", description: "buttery flaky pastry", descriptionAr: "معجنات زبدية مقرمشة", image: IMG.croissant, available: true, featured: false, variants: [v("Regular", 75, null, "عادي")] },
      { id: 34, sectionId: 8, name: "Chocolate Croissant", nameAr: "كرواسون بالشوكولاتة", description: "filled with rich chocolate", descriptionAr: "محشو بشوكولاتة غنية", image: IMG.croissant, available: true, featured: false, variants: [v("Regular", 95, null, "عادي")] },
      { id: 35, sectionId: 8, name: "Lotus Croissant", nameAr: "كرواسون لوتس", description: "filled with lotus biscoff cream", descriptionAr: "محشو بكريمة لوتس بيسكوف", image: IMG.croissant, available: true, featured: false, variants: [v("Regular", 95, null, "عادي")] },
      { id: 36, sectionId: 8, name: "Pistachio Croissant", nameAr: "كرواسون فستق", description: "filled with pistachio cream", descriptionAr: "محشو بكريمة فستق", image: IMG.croissant, available: true, featured: false, variants: [v("Regular", 135, null, "عادي")] },
      { id: 37, sectionId: 8, name: "Almond Croissant", nameAr: "كرواسون لوز", description: "topped with almond flakes", descriptionAr: "مغطى برقائق اللوز", image: IMG.croissant, available: true, featured: false, variants: [v("Regular", 135, null, "عادي")] },
      { id: 38, sectionId: 8, name: "Cheese Croissant", nameAr: "كرواسون جبنة", description: "filled with creamy cheese", descriptionAr: "محشو بجبنة كريمية", image: IMG.croissant, available: true, featured: false, variants: [v("Regular", 95, null, "عادي")] },
      { id: 39, sectionId: 8, name: "Turkey Croissant", nameAr: "كرواسون تركي", description: "savory turkey filling", descriptionAr: "حشوة ديك رومي لذيذة", image: IMG.croissant, available: true, featured: false, variants: [v("Regular", 135, null, "عادي")] },
  ...
  function save(data) {
    ...
    // credentials: "include" sends the httpOnly session cookie set by /api/login;
    // the server rejects this with 401 if the session is missing or expired.
    fetch(API_URL, {
      method: "POST",
      credentials: "include",
  ...
  // isAuthenticated() is a UI-only convenience flag — it just controls
  // whether the dashboard shell shows on page load without waiting on a
  // network round trip. It grants nothing by itself: every admin write
  // (POST /api/menu, POST /api/upload) is independently checked server-side
  // against the real session cookie, so a forged sessionStorage value can't
  // be used to bypass anything.
  function isAuthenticated() {
    return sessionStorage.getItem(AUTH_KEY) === "true";
  }

  // Credentials are verified server-side by /api/login (see api/login.js),
  // which sets an httpOnly session cookie on success. The browser never
  // handles a secret here.
